...
The dataset contains monthly employment rates in the United States from 1990 to 2025. Each record corresponds to a specific year and month, reporting the seasonally adjusted unemployment rate as a percentage of the civilian labor force. The dataset is sourced from US Bureau of Labor Statistics and accessed via [government website](https://data.bls.gov/timeseries/LNS14000000?). The Bureau of Labor Statistics (BLS) draws data from the Current Population Survey (CPS), administrative records, and Current Employment Statistics (CES). Gathering data from multiple sources enables the BLS to accurately cover employment rates and capture the full picture. 
...
Our exploratory analysis seeks to quantify and visualize employment trends in the United States. With this process, we hope to better understand what factors influence the job market. 
In particular, we are interested if major geopolitical events greatly influence employment rates and the types of jobs available. Our focus on examining recent data from the Great Recession and COVID-19 enables us to see how the world responds to major historical events. Additionally, we aim to investigate whether there have been long-term shifts in employment patterns.
...
setwd("/Users/leilabuchan/Downloads/")
emp2 <- read_excel("SeriesReport1990-2025.xlsx", skip= 10)
```

## Cleaning and Filtering Data

Before analyzing a data set, it is imperative to see the structure of the data set, therefore below, you can see the top 6 rows (1990-1995) and the different months according to each year as the columns, with different unemployment rates as the value.  In order to visualize it so the independent variable is the dates, we do have to restructure the data set to do so, which we do in this section. 
...
This step prepares the data set for analysis by filtering the different years of one data set to match that of the second data set. 

## Restructuing for Monthly Analysis 

We first used the pivot_longer() function and reshaped the format of the data set from wide to long, so that we can plot multiple employment data sets on a single graph, giving us more efficiency of providing information. It converts the months from the emp2 data into a shared column called “months” with the unemployment rates as values. This will help to visualize the data so the independent variable is the months of each year. We then used the mutate() function to change different variables into their respective string types. We also use the function to change the month abbreviations (Jan, Feb,...) into their respective numeric numbers, with Jan being 1, Feb being 2 and so on, so that we can visualize the graph by color depending on the different months. We then add another variable as a column called “month_name” that stores the month abbreviations, so that the heatmap can be visualized by month name rather than the number. We also use the drop_na() feature to rid of all NAs, and make the data as clean as possible

```{r}
#creating another data set that looks at each individual months as the independent variable, and the valyes to be rate. 
...
# National Unemployment Overview (1990-Present)

## Yearly Trends Showing Cyclical Peaks
...
ggplot(monthly, aes(x = Year, y = rate)) +
...
This line plot reveals that throughout recent decades there have been both periods of very high and very low unemployment. One noticeable spike is in 2010, likely due to the great recession. Another noticeable spike is in 2020, probably a result of the COVID-19 pandemic.  It also emphasizes the volatility related to the job market, as there is no consistency in the unemployment rate across decades or even years. This can also be viewed as a way to show the market resilience to global events, as eventually the unemployment rate does go down. It seems the unemployment rates are quite cyclical as within most decades or chunks of time there are reported high and low percentages of unemployment. With these peaks, we look to study more specifics of each crises, and how they managed to recover from such high peaks.   

# The Great Recession (2006-2014) 
...
## Peak Unemployment Occured after Recessions Official End
```{r}
...
## Summary Statistics for Great Recession 
...
From the central tendencies of the Great Recession, we see that mean unemployment rates were highest from 2009-2012 with a very gradual decrease. Although the recession began in 2007, unemployment didnt hit numbers until 2009, however, that was when the recession was considered "done" on June 2009. We see that unemployment spikes when the recession was considered over, which I find interesting and it took a very long time to recover, with each year being a slight decrease in the average unemployment rates. If we wanted to look at the median only, we also see very similar trends. 

# COVID-19 Pandemic (2019-2024)

## Monthly Trends Showing Rapid Rise and Recovery
...
ggplot(monthly_covid, aes(x = date, y = rate, color = month)) +
  ...
  theme_minimal()
```
# Peak Unemployment in April 2020
```{r}
...
# Summary Statistics for COVID-19
```{r}
...
While our data set offers a glimpse into employment rates in the United States throughout years, there are still gaps in the review. Our analysis briefly mentions the relationship between unemployment rates and major historical events. Further, the data set provides a national employment statistic preventing us from examining regional differences. Exploring subsets of unemployment data can inform us to which types of jobs are most resilient to job loss. Building off our strong foundational analysis can lead to better understanding of United States unemployment trends and influences. Specifically, understanding how different regions are impacted and respond to different types of major historical events. With future analysis we hope to answer 1) How do unemployment rates vary across states and regions? And 2) Are some regions and jobs more vulnerable to events?. 
