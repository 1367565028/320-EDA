...
# Data Visualization for Unemployment Rates (Year)
...
# Data Visualization of Unemployment for COVID-19 (Monthly)
...
# Central Tendencies for each data
```{r}
#central tendencies for unemployment over the years 
employment %>%
  group_by(year)%>%
  summarise(
    unemp_mean = mean(unemployed, na.rm = TRUE),
    unemp_median = median(unemployed, na.rm = TRUE),
    unemp_min = min(unemployed, na.rm = TRUE),
    unemp_max = max(unemployed, na.rm = TRUE),
  )
```

From the original data set with the unemployment from 1990-2024, we see the different means, median, min and max by each year, so that we can see them for all years. However, this does not provide enough information so we look to the other data set to show nuances in the years during the Great Recession and COVID-19. 
